...
#include "LinuxFactoryDataProvider.h"
...
#include <platform/Linux/CHIPDevicePlatformConfig.h>
...
CHIP_ERROR FactoryDataProvider::ReadFileBytes(std::vector<uint8_t> &outBuffer, const std::string &filepath)
{
    ...
    std::vector<uint8_t> read_buffer((std::istreambuf_iterator<char>(file)), std::istreambuf_iterator<char>());

    outBuffer = std::move(read_buffer);
...
CHIP_ERROR FactoryDataProvider::Init(DeviceAttestationCredentialsFilepath DacFilepath)
{
    CHIP_ERROR err = CHIP_NO_ERROR;

    // Read certs and keys from files
    err = ReadFileBytes(mFactoryData.dac.dac_cert.value, DacFilepath.dac_cert_filepath);
    if (err != CHIP_NO_ERROR)
    {
        ChipLogError(DeviceLayer, "Read dac_cert failed!");
        return err;
    }
    mFactoryData.dac.dac_cert.len = mFactoryData.dac.dac_cert.value.size();
    ChipLogProgress(DeviceLayer, "dac_cert of length: %zu retrieved successfully", mFactoryData.dac.dac_cert.len);

    err = ReadFileBytes(mFactoryData.dac.dac_key.value, DacFilepath.dac_key_filepath);
    if (err != CHIP_NO_ERROR)
    {
        ChipLogError(DeviceLayer, "Read dac_key failed!");
        return err;
    }
    mFactoryData.dac.dac_key.len = mFactoryData.dac.dac_key.value.size();
    ChipLogProgress(DeviceLayer, "dac_key of length: %zu retrieved successfully", mFactoryData.dac.dac_key.len);

    err = ReadFileBytes(mFactoryData.dac.pai_cert.value, DacFilepath.pai_cert_filepath);
    if (err != CHIP_NO_ERROR)
    {
        ChipLogError(DeviceLayer, "Read pai_cert failed!");
        return err;
    }
    mFactoryData.dac.pai_cert.len = mFactoryData.dac.pai_cert.value.size();
    ChipLogProgress(DeviceLayer, "pai_cert of length: %zu retrieved successfully", mFactoryData.dac.pai_cert.len);

    err = ReadFileBytes(mFactoryData.dac.cd.value, DacFilepath.cd_filepath);
    if (err != CHIP_NO_ERROR)
    {
        ChipLogError(DeviceLayer, "Read cd failed!");
        return err;
    }
    mFactoryData.dac.cd.len = mFactoryData.dac.cd.value.size();
    ChipLogProgress(DeviceLayer, "cd of length: %zu retrieved successfully", mFactoryData.dac.cd.len);
...
CHIP_ERROR FactoryDataProvider::GetCertificationDeclaration(MutableByteSpan & outBuffer)
{
    ...
    if (kReadFromFlash)
    {
        ...
        ReturnErrorCodeIf(mFactoryData.dac.cd.value.empty(), CHIP_ERROR_PERSISTED_STORAGE_VALUE_NOT_FOUND);

        // memcpy(outBuffer.data(), mFactoryData.dac.cd.value, mFactoryData.dac.cd.len);
        // std::copy(mFactoryData.dac.cd.value.begin(), mFactoryData.dac.cd.value.end(), outBuffer.data());

        // outBuffer.reduce_size(mFactoryData.dac.cd.len);
        // err = CHIP_NO_ERROR;

        err = CopySpanToMutableSpan(ByteSpan(mFactoryData.dac.cd.value.data(), mFactoryData.dac.cd.len), outBuffer);
    }
    else
    {
...
CHIP_ERROR FactoryDataProvider::GetDeviceAttestationCert(MutableByteSpan & outBuffer)
{
    ...
    if (kReadFromFlash)
    {
        ...
        ReturnErrorCodeIf(mFactoryData.dac.dac_cert.value.empty(), CHIP_ERROR_PERSISTED_STORAGE_VALUE_NOT_FOUND);

        // memcpy(outBuffer.data(), mFactoryData.dac.dac_cert.value, mFactoryData.dac.dac_cert.len);
        // std::copy(mFactoryData.dac.dac_cert.value.begin(), mFactoryData.dac.dac_cert.value.end(), outBuffer.data());

        // outBuffer.reduce_size(mFactoryData.dac.dac_cert.len);
        // err = CHIP_NO_ERROR;

        err = CopySpanToMutableSpan(ByteSpan(mFactoryData.dac.dac_cert.value.data(), mFactoryData.dac.dac_cert.len), outBuffer);
    }
    else
    {
...
CHIP_ERROR FactoryDataProvider::GetProductAttestationIntermediateCert(MutableByteSpan & outBuffer)
{
    ...
    if (kReadFromFlash)
    {
        ...
        ReturnErrorCodeIf(mFactoryData.dac.pai_cert.value.empty(), CHIP_ERROR_PERSISTED_STORAGE_VALUE_NOT_FOUND);

        // memcpy(outBuffer.data(), mFactoryData.dac.pai_cert.value, mFactoryData.dac.pai_cert.len);
        // std::copy(mFactoryData.dac.pai_cert.value.begin(), mFactoryData.dac.pai_cert.value.end(), outBuffer.data());

        // outBuffer.reduce_size(mFactoryData.dac.pai_cert.len);
        // err = CHIP_NO_ERROR;

        err = CopySpanToMutableSpan(ByteSpan(mFactoryData.dac.pai_cert.value.data(), mFactoryData.dac.pai_cert.len), outBuffer);
    }
    else
    {
...
CHIP_ERROR FactoryDataProvider::SignWithDeviceAttestationKey(const ByteSpan & messageToSign, MutableByteSpan & outSignBuffer)
{
    ...
    {
        ReturnErrorCodeIf(mFactoryData.dac.dac_cert.value.empty(), CHIP_ERROR_PERSISTED_STORAGE_VALUE_NOT_FOUND);
        ReturnErrorCodeIf(mFactoryData.dac.dac_key.value.empty(), CHIP_ERROR_PERSISTED_STORAGE_VALUE_NOT_FOUND);
        // Extract public key from DAC cert.
        ByteSpan dacCertSpan{ reinterpret_cast<uint8_t *>(mFactoryData.dac.dac_cert.value.data()), mFactoryData.dac.dac_cert.len };
        chip::Crypto::P256PublicKey dacPublicKey;

        ReturnErrorOnFailure(chip::Crypto::ExtractPubkeyFromX509Cert(dacCertSpan, dacPublicKey));
        ReturnErrorOnFailure(
            LoadKeypairFromRaw(ByteSpan(reinterpret_cast<uint8_t *>(mFactoryData.dac.dac_key.value.data()), mFactoryData.dac.dac_key.len),
                               ByteSpan(dacPublicKey.Bytes(), dacPublicKey.Length()), keypair));

        // Crypto::P256SerializedKeypair serialized_keypair;
        // serialized_keypair.SetLength(mFactoryData.dac.dac_key.len);
        // std::copy(mFactoryData.dac.dac_key.value.begin(), mFactoryData.dac.dac_key.value.end(), serialized_keypair.Bytes());
...
CHIP_ERROR FactoryDataProvider::GetSpake2pSalt(MutableByteSpan & saltBuf)
{
    ...
    if (kReadFromFlash)
    {
        ...
        memcpy(saltB64, mFactoryData.cdata.spake2_salt.value.data(), saltB64Len);
        err = CHIP_NO_ERROR;
    }
    else
    {
...
CHIP_ERROR FactoryDataProvider::GetSpake2pVerifier(MutableByteSpan & verifierBuf, size_t & verifierLen)
{
    ...
    if (kReadFromFlash)
    {
        ...
        memcpy(verifierB64, mFactoryData.cdata.spake2_verifier.value.data(), verifierB64Len);
        err = CHIP_NO_ERROR;
    }
    else
    {
    ...
    ReturnErrorOnFailure(err);
    verifierLen = chip::Base64Decode32(verifierB64, static_cast<uint32_t>(verifierB64Len), reinterpret_cast<uint8_t *>(verifierB64));
...
CHIP_ERROR FactoryDataProvider::GetVendorName(char * buf, size_t bufSize)
{
    ...
    if (kReadFromFlash)
    {
        ...
        ReturnErrorCodeIf(mFactoryData.dii.vendor_name.value.empty(), CHIP_ERROR_PERSISTED_STORAGE_VALUE_NOT_FOUND);
        // memcpy(buf, mFactoryData.dii.vendor_name.value, mFactoryData.dii.vendor_name.len);
        std::copy(mFactoryData.dii.vendor_name.value.begin(), mFactoryData.dii.vendor_name.value.end(), buf);
...
CHIP_ERROR FactoryDataProvider::GetProductName(char * buf, size_t bufSize)
{
    ...
    if (kReadFromFlash)
    {
        ...
        ReturnErrorCodeIf(mFactoryData.dii.product_name.value.empty(), CHIP_ERROR_PERSISTED_STORAGE_VALUE_NOT_FOUND);
        // memcpy(buf, mFactoryData.dii.product_name.value, mFactoryData.dii.product_name.len);
        std::copy(mFactoryData.dii.product_name.value.begin(), mFactoryData.dii.product_name.value.end(), buf);
...
CHIP_ERROR FactoryDataProvider::GetSerialNumber(char * buf, size_t bufSize)
{
    ...
    if (kReadFromFlash)
    {
        ...
        ReturnErrorCodeIf(mFactoryData.dii.serial_num.value.empty(), CHIP_ERROR_PERSISTED_STORAGE_VALUE_NOT_FOUND);
        // memcpy(buf, mFactoryData.dii.serial_num.value, mFactoryData.dii.serial_num.len);
        std::copy(mFactoryData.dii.serial_num.value.begin(), mFactoryData.dii.serial_num.value.end(), buf);
...
CHIP_ERROR FactoryDataProvider::GetManufacturingDate(uint16_t & year, uint8_t & month, uint8_t & day)
{
    ...
    if (kReadFromFlash)
    {
        ...
        year = static_cast<uint16_t>(strtoul((char *) mFactoryData.dii.mfg_date.value.data(), &parseEnd, 10));
        VerifyOrExit(parseEnd == (char *) mFactoryData.dii.mfg_date.value.data() + 4, err = CHIP_ERROR_INVALID_ARGUMENT);

        // Cast does not lose information, because we then check that we only parsed
        // 2 digits, so our number can't be bigger than 99.
        month = static_cast<uint8_t>(strtoul((char *) mFactoryData.dii.mfg_date.value.data() + 5, &parseEnd, 10));
        VerifyOrExit(parseEnd == (char *) mFactoryData.dii.mfg_date.value.data() + 7, err = CHIP_ERROR_INVALID_ARGUMENT);

        // Cast does not lose information, because we then check that we only parsed
        // 2 digits, so our number can't be bigger than 99.
        day = static_cast<uint8_t>(strtoul((char *) mFactoryData.dii.mfg_date.value.data() + 8, &parseEnd, 10));
        VerifyOrExit(parseEnd == (char *) mFactoryData.dii.mfg_date.value.data() + 10, err = CHIP_ERROR_INVALID_ARGUMENT);
...
exit:
    ...
    {
        ChipLogError(DeviceLayer, "Invalid manufacturing date: %s", mFactoryData.dii.mfg_date.value.data());
    }
    return err;
}
...
CHIP_ERROR FactoryDataProvider::GetHardwareVersionString(char * buf, size_t bufSize)
{
    ...
    if (kReadFromFlash)
    {
        ...
        ReturnErrorCodeIf(mFactoryData.dii.hw_ver_string.value.empty(), CHIP_ERROR_PERSISTED_STORAGE_VALUE_NOT_FOUND);
        // memcpy(buf, mFactoryData.dii.hw_ver_string.value, mFactoryData.dii.hw_ver_string.len);
        std::copy(mFactoryData.dii.hw_ver_string.value.begin(), mFactoryData.dii.hw_ver_string.value.end(), buf);
...
CHIP_ERROR FactoryDataProvider::GetRotatingDeviceIdUniqueId(MutableByteSpan & uniqueIdSpan)
{
    ...
    if (kReadFromFlash)
    {
        ...
        ReturnErrorCodeIf(mFactoryData.dii.rd_id_uid.value.empty(), CHIP_ERROR_PERSISTED_STORAGE_VALUE_NOT_FOUND);
        // memcpy(uniqueIdSpan.data(), mFactoryData.dii.rd_id_uid.value, mFactoryData.dii.rd_id_uid.len);
        std::copy(mFactoryData.dii.rd_id_uid.value.begin(), mFactoryData.dii.rd_id_uid.value.end(), uniqueIdSpan.data());
        err = CHIP_NO_ERROR;
    }
    else
    {
