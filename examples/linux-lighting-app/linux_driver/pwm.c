...
int pwm_open(pwm_t *pwm, unsigned int chip, unsigned int channel) {
    ...
    if (stat(channel_path, &stat_buf) < 0) {
        ...
        len = (size_t)snprintf(buf, sizeof(buf), "%u\n", channel);
    ...
    pwm->chip = (ssize_t)chip;
    pwm->channel = (ssize_t)channel;
...
int pwm_close(pwm_t *pwm) {
    ...
    if (pwm->channel == ((ssize_t) -1))
        return 0;

    /* Unexport the PWM */
    snprintf(path, sizeof(path), "/sys/class/pwm/pwmchip%zd/unexport", pwm->chip);

    len = (size_t)snprintf(buf, sizeof(buf), "%zd\n", pwm->channel);
...
static int pwm_read_attribute(pwm_t *pwm, const char *name, char *buf, size_t len) {
    ...
    snprintf(path, sizeof(path), "/sys/class/pwm/pwmchip%zd/pwm%zd/%s", pwm->chip, pwm->channel, name);
...
int pwm_set_period_ns(pwm_t *pwm, uint64_t period_ns) {
    ...
    len = (size_t)snprintf(buf, sizeof(buf), "%" PRId64 "\n", period_ns);
...
ssize_t pwm_chip(pwm_t *pwm) {
    return pwm->chip;
}

ssize_t pwm_channel(pwm_t *pwm) {
    return pwm->channel;
}
...
int pwm_tostring(pwm_t *pwm, char *str, size_t len) {
    ...
    return snprintf(str, len, "PWM %zd, chip %zd (period=%s sec, duty_cycle=%s%%, polarity=%s, enabled=%s)", pwm->channel, pwm->chip, period_str, duty_cycle_str, polarity_str, enabled_str);
}
